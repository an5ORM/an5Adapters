...
exports.substParams = substParams;
exports.matchRowSQL = matchRowSQL;
exports.extractColsAndVals = extractColsAndVals;
const helpers_1 = require("./helpers");
const retry_1 = require("./retry");
async function execQuery(adapter, query, params, _modelMap, _sheetId) {
    const q = query.trim();
    const upper = q.toUpperCase();
    if (upper.startsWith('SELECT'))
        return execSelect(adapter, q, params);
    if (upper.startsWith('INSERT'))
        return execInsert(adapter, q, params);
    if (upper.startsWith('UPDATE'))
        return execUpdate(adapter, q, params);
    if (upper.startsWith('DELETE'))
        return execDelete(adapter, q, params);
    // Fallback: treat as raw range or sheet name
    const range = q.includes('!') ? q : (0, helpers_1.esc)(q) + '!A:ZZ';
...
function matchRowSQL(row, whereText, values) {
    if (!whereText)
        return true;
    ...
    for (const part of parts) {
        ...
        switch (op) {
            ...
            case '<=':
                ...
                break;
            case 'LIKE':
                if (!cell || !String(cell).toLowerCase().includes(String(val).toLowerCase().replace(/%/g, '')))
                    return false;
                break;
            case 'CONTAINS':
                if (!cell || !String(cell).includes(String(val)))
                    return false;
                break;
        }
    }
...
function extractColsAndVals(sql, params) {
    const cols = [];
    const vals = [];
...
async function execSelect(adapter, query, params) {
    const m = query.match(/SELECT\s+(.+?)\s+FROM\s+(?:'([^']+)'|"([^"]+)"|(\S+))\s*(?:WHERE\s+(.+))?$/i);
    if (!m) {
        // Try reading as raw range
        const raw = await adapter.readRange((0, helpers_1.esc)(query.replace(/^SELECT\s+\*\s+FROM\s+/i, '').trim()) + '!A:ZZ');
        const headers = raw[0] || [];
    ...
    const raw = await adapter.readRange((0, helpers_1.esc)(sheetName) + '!A:ZZ');
    if (raw.length < 1)
        return [];
    const headers = raw[0];
    ...
    if (!whereClause)
        return selectAll ? rows : rows.map(r => pickFields(r, fieldList, headers));
...
function pickFields(row, fieldList, headers) {
    const fields = fieldList.split(',').map(f => f.trim().replace(/[[\]]/g, ''));
    const obj = {};
    fields.forEach(f => { obj[f] = row[f]; });
    return obj;
}
async function execInsert(adapter, query, params) {
    ...
    if (cols.length === 0)
        throw new Error('Cannot extract columns from INSERT');
    // Ensure headers exist
    const raw = await adapter.readRange((0, helpers_1.esc)(sheetName) + '!A1:ZZ1');
    ...
    if (existingHeaders.length === 0) {
        ...
        await (0, retry_1.withRetry)(() => api.spreadsheets.values.update({
            ...
            range: (0, helpers_1.esc)(sheetName) + '!A1',
    ...
    await adapter.appendRange((0, helpers_1.esc)(sheetName) + '!A:A', [rowVals]);
    return 1;
}
async function execUpdate(adapter, query, params) {
    ...
    // Read data
    const range = (0, helpers_1.esc)(sheetName) + '!A:ZZ';
    const raw = await adapter.readRange(range);
    if (raw.length < 1)
        return 0;
    const headers = raw[0];
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        row.__row = i + 2;
        if (whereClause && !matchRowSQL(row, whereSubst, whereVals))
            continue;
        // Apply SET values
        ...
        await adapter.writeRange((0, helpers_1.esc)(sheetName) + `!A${row.__row}`, [updated]);
        count++;
    }
    return count;
}
async function execDelete(adapter, query, params) {
